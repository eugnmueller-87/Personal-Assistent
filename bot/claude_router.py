...
def route(user_message: str, user_id: str = "default") -> str:
    ...
    system = (
        ...
        "- build_miro_board: ALWAYS call this for ANY Miro request — comparisons, landscapes, ratings, "
        "overviews, maps, visualizations. Miro IS available and fully working. NEVER say it is not. "
        "board_type='landscape' for any supplier/company/comparison request. "
        "board_type='signals' only when the user explicitly asks for a signals board. "
        "For comparison requests (e.g. 'compare Google vs AWS'), use board_type='landscape' and set "
        "category to the relevant market (e.g. 'Cloud & Infrastructure') — the board will show all "
        "tracked players in that space including both companies. After calling the tool, tell the user "
        "what the board shows and give them the URL. NEVER refuse a Miro request — always call the tool "
        "and let it respond. If it returns an error, report the error. If it returns a URL, share it.\n"
